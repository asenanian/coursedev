...
public class VelocityAction extends ActionUtility implements IAction {
	
	private IGameObject objectOnDown;
	private Field fieldOnDown;
	
	
	public VelocityAction(GameManager manager, GameRenderer renderer){
	...
	public boolean actOnTouchUp(float [] mousePos) {
		if (objectOnDown != null){
			manager.addVelocityToObject(objectOnDown, mousePos); 
		} else if (fieldOnDown != null){
			manager.addVelocityToField(fieldOnDown, mousePos);
		}
		renderer.endBuilder();
		return true;
	}
